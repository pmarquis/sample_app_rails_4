...
//The drawing of the histogram has been broken out from the data retrial 
  ...
  var draw_histogram = function(reference, pos_data){

      pos_data = [4, 8, 15, 16, 23, 42];
  ...
  elt_chart_dict = {};
  div_name = "";
  elt_chart_dict["div#graph"] = draw_histogram;
  elt_chart_dict["div#graph1"] = draw_histogram;

  for(key in eval(elt_chart_dict)) {
    if( $(key).length ) {
      console.debug(key);
      console.debug(elt_chart_dict[key]);
      div_name = key;
      $.ajax({
        type: "GET",
        ...
        success: function (received_data) {
          elt_chart_dict["div#graph"](div_name, received_data);
        },
